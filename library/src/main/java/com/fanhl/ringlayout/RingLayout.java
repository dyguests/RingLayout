...
public class RingLayout extends ViewGroup {
    private static final String TAG = RingLayout.class.getSimpleName();
    ...
    public RingLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.RingLayout);

        mStartAngle = typedArray.getInt(R.styleable.RingLayout_startAngle, 0) % ANGLE_360;
        mEndAngle = typedArray.getInt(R.styleable.RingLayout_endAngle, mStartAngle) % ANGLE_360;

        mRotateDirection = typedArray.getInt(R.styleable.RingLayout_rotateDirection, CLOCKWISE);

        mPathShape = typedArray.getInt(R.styleable.RingLayout_pathShape, CIRCLE);
    }
